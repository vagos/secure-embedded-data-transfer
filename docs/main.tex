...
The sensor allows for both digital and analog output, with the digital output being a
on/off signal for which a limit value can be set via the potensiometer and the analog output being a voltage signal between 0-5V that is proportional to the sound intensity.
...
The microphone's input sensitivity according the data sheets is 48-66dB, around the level of a normal conversation.
...
MQTT three quality of service (QoS) levels:
...
However, this kind of program transformation results in noticable performance losses.
...
code, even though progress is being done in VM-obfuscation reverse engineering \cite{VMReversing, UnpackingVirtualizationObfuscators}.
...
to secure the various layer of our system.
...
% \begin{tikzpicture}[
%     block/.style={rectangle, draw, text width=\columnwidth, text centered, rounded corners, minimum height=1em},
%     node distance=0.1cm,
%     font=\footnotesize]

%   % Define nodes with different colors
%   \node [block, fill=blue!30] (transit) {\textbf{Data in Transit}\\Security: SSL/TLS\\Attacks: Interception, MITM};
%   \node [block, fill=green!30, above=of transit] (rest) {Data at Rest\\Security: AES Encryption\\Attacks: Unauthorized Access, Data Leakage};
%   \node [block, fill=orange!30, above=of rest] (logicConf) {Program Logic Confidentiality\\Security: Flash Encryption, Obfuscation, Virtualization\\Attacks: Reverse Engineering, IP Theft};
%   \node [block, fill=purple!30, above=of logicConf] (logicInt) {Program Logic Integrity\\Security: Flash Encryption, Stack Overflow Checking\\Attacks: Code Injection, Buffer Overflow};

% \end{tikzpicture}
% \end{adjustbox}
% \caption{Embedded System Security Layers}
...
\appendix{Reverse Engineering the ESP32 Firmware}
